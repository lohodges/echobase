...
# Explanation: Static files are the easy win—echobase caches them like hyperfuel for speed.
resource "aws_cloudfront_cache_policy" "echobase_cache_static01" {
  ...
  parameters_in_cache_key_and_forwarded_to_origin {
    # Explanation: Static should not vary on cookies—echobase refuses to cache 10,000 versions of a PNG.
...
# Explanation: APIs are dangerous to cache by accident—echobase disables caching until proven safe.
resource "aws_cloudfront_cache_policy" "echobase_cache_api_disabled01" {
...
# Explanation: Origins need context—echobase forwards what the app needs without polluting the cache key.
resource "aws_cloudfront_origin_request_policy" "echobase_orp_api01" {
...
# Explanation: Static origins need almost nothing—echobase forwards minimal values for maximum cache sanity.
resource "aws_cloudfront_origin_request_policy" "echobase_orp_static01" {
...
# Explanation: Make caching intent explicit—echobase stamps Cache-Control so humans and CDNs agree.
resource "aws_cloudfront_response_headers_policy" "echobase_rsp_static01" {
