# Explanation: echobase only opens the hangar to CloudFront — everyone else gets the Wookiee roar.
...
# Explanation: This is echobase’s secret handshake — if the header isn’t present, you don’t get in.
...
# Explanation: ALB checks for echobase’s secret growl — no growl, no service.
resource "aws_lb_listener_rule" "echobase_require_origin_header01" {
  ...
  condition {
    http_header {
      http_header_name = "X-echobase-Growl"
      values           = [random_password.echobase_origin_header_value01.result]
    }
  }
}

# Explanation: If you don’t know the growl, you get a 403 — echobase does not negotiate.
