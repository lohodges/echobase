...
# Default CloudFront standard log field order (V2 logs omit the #Fields: header)
DEFAULT_FIELDS = [
...
DEFAULT_FIELD_INDEX = {name: idx for idx, name in enumerate(DEFAULT_FIELDS)}
...
def count_standard_log_files(file_paths: List[str]) -> Dict[str, int]:
    ...
    for path in file_paths:
        field_index: Dict[str, int] = dict(DEFAULT_FIELD_INDEX)

        with open_maybe_gzip(path) as f:
            for line in f:
                if line.startswith("#Fields:"):
                    # Override default with explicit header if present (legacy V1 logs)
...
def main() -> int:
    ...
    ap.add_argument("--latest", type=int, default=30, help="Download and analyze the latest N log objects (default: 3)")
