...
json_escape() { sed -e 's/\\/\\\\/g' -e 's/"/\\"/g' -e 's/\t/\\t/g' -e 's/\r/\\r/g' -e ':a;N;$!ba;s/\n/\\n/g'; }
...
cf_web_acl_id="$(aws cloudfront get-distribution --id "$CF_DISTRIBUTION_ID" --query "Distribution.DistributionConfig.WebACLId" --output text 2>/dev/null || echo "")"
...
if [[ "$REQUIRE_WAF_ASSOCIATION" == "true" ]]; then
  [[ -n "$cf_web_acl_id" && "$cf_web_acl_id" != "None" ]] \
