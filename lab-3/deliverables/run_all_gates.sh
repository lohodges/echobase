...
if [[ "$REQUIRE_WAF_ASSOCIATION" == "true" ]]; then
  if [[ -n "$cf_web_acl_id" && "$cf_web_acl_id" != "None" ]]; then
    add_detail "PASS: WAF WebACL is associated with CloudFront."
    if [[ -n "$WAF_WEB_ACL_ARN" && "$cf_web_acl_id" != "$WAF_WEB_ACL_ARN" ]]; then
      add_warning "WARN: Associated WebACL ARN differs from WAF_WEB_ACL_ARN input (expected=$WAF_WEB_ACL_ARN, actual=$cf_web_acl_id)."
    fi
...
if [[ -n "$logging_bucket" && "$logging_bucket" != "None" ]]; then
  ...
  add_detail "PASS: CloudFront legacy standard logging enabled (bucket=$logging_bucket)."
else
  # Fall back: check for CloudWatch Vended Log Delivery (standard logging v2)
  cf_dist_arn="$(aws cloudfront get-distribution --id "$CF_DISTRIBUTION_ID" \
    --query "Distribution.ARN" --output text 2>/dev/null || echo "")"
  if [[ -n "$cf_dist_arn" && "$cf_dist_arn" != "None" ]]; then
    vended_log_count="$(aws logs describe-delivery-sources --region "$CF_ACM_REGION" \
      --query "length(deliverySources[?resourceArn=='$cf_dist_arn'])" --output text 2>/dev/null || echo "0")"
    if [[ "$vended_log_count" != "0" && "$vended_log_count" != "None" ]]; then
      logging_enabled="true"
      add_detail "PASS: CloudFront standard logging v2 enabled via CloudWatch Log Delivery (delivery_sources=$vended_log_count)."
    fi
  fi

  if [[ "$logging_enabled" != "true" ]]; then
    if [[ "$REQUIRE_LOGGING" == "true" ]]; then
      add_failure "FAIL: CloudFront logging not enabled (no legacy Logging.Bucket, no CloudWatch Log Delivery found)."
    else
...
if [[ -n "$LOG_BUCKET" ]]; then
  ...
  if [[ "$logging_enabled" == "true" && -n "$logging_bucket" && "$logging_bucket" != "None" ]]; then
