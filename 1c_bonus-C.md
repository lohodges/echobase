Below is the Route53 add-on for Bonus-B (Hosted Zone + ACM DNS validation records + app.echobase.click ALIAS → ALB).
...
############################################
# ALIAS record: app.echobase.click -> ALB
############################################

# Explanation: This is the holographic sign outside the cantina—app.echobase.click points to your ALB.
...
1) Confirm hosted zone exists (if managed)
  aws route53 list-hosted-zones-by-name \
    --dns-name echobase.click \
    --query "HostedZones[].Id"
```
1c_terrraform lhj-keepItSimple  ? ❯ aws route53 list-hosted-zones-by-name \
    --dns-name echobase.click \
    --query "HostedZones[].Id"
[
    "/hostedzone/Z0828030PI6PCZKRD9SW"
]
```
...
2) Confirm app record exists
  ...
  --query "ResourceRecordSets[?Name=='app.echobase.click.']"
```
1c_terrraform lhj-keepItSimple  ? ✗ aws route53 list-resource-record-sets   --hosted-zone-id Z0828030PI6PCZKRD9SW   --query "ResourceRecordSets[?Name=='app.echobase.click.']"
[
    {
        ...
        "AliasTarget": {
            ...
            "DNSName": "echobase-alb01-144478447.us-east-2.elb.amazonaws.com.",
...
```
1c_terrraform lhj-keepItSimple  ? ❯ aws acm describe-certificate \
  --certificate-arn arn:aws:acm:us-east-2:746669200167:certificate/889e1272-be2d-4fdf-b73f-1bda5219e846 \
  --query "Certificate.Status"
"ISSUED"
```
...
```
1c_terrraform lhj-keepItSimple  ? ❯ curl -I https://app.echobase.click
HTTP/2 200
date: Tue, 20 Jan 2026 18:01:08 GMT
